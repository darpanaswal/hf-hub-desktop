...
def patched_upload(
    ...
    preresolved_files=None,   # list of Path objects when uploading multiple selected files
):
    ...
    try:
        ...
        if preresolved_files is not None:
            all_files = preresolved_files
        else:
...
class UploadFolderRequest(BaseModel):
    local_path: str                        # single path (file or folder)
    local_paths: Optional[List[str]] = None  # multiple files (overrides local_path)
    repo_id: str
    repo_type: str = "model"
    path_in_repo: str = ""
...
async def upload_folder(req: UploadFolderRequest):
    # Support uploading multiple individually selected files
    if req.local_paths:
        missing = [p for p in req.local_paths if not os.path.exists(p)]
        if missing:
            raise HTTPException(status_code=400, detail=f"Files not found: {', '.join(missing)}")
        all_files = [Path(p) for p in req.local_paths if Path(p).is_file()]
        # Use the common parent as the base for path_in_repo calculation
        try:
            base = Path(os.path.commonpath([str(f) for f in all_files]))
            if base.is_file():
                base = base.parent
        except ValueError:
            base = all_files[0].parent
        path = str(base)
    else:
        path = req.local_path
        ...
        try:
            await loop.run_in_executor(
                ...
                lambda: patched_upload(
                    api=_api(),
                    local_path=path,
                    preresolved_files=all_files if req.local_paths else None,
                    repo_id=req.repo_id,
