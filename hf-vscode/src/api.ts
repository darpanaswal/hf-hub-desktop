...
export class HfApi {
  ...
  async startDownload(params: { repo_id: string; revision?: string; local_dir?: string; create_repo_subfolder?: boolean; filenames?: string[] }): Promise<any> {
  ...
  async listRepoFiles(repoId: string, revision = 'main'): Promise<any> {
    return this.request('GET', `/cache/files?repo_id=${encodeURIComponent(repoId)}&revision=${revision}`)
  }
}
