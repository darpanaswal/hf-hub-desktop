  ...
  private constructor(panel: vscode.WebviewPanel, private api: HfApi, initialPath?: string) {
    ...
    this._panel.webview.onDidReceiveMessage(async (msg) => {
      switch (msg.type) {
        ...
        case 'browse': {
          // If browseMode is 'files', allow multi-select of files only
          // Otherwise allow picking a folder or a single file
          if (msg.browseMode === 'files') {
            ...
            if (result?.length) {
              const paths = result.map((u: any) => u.fsPath)
              this._panel.webview.postMessage({ type: 'setFiles', paths })
            }
          } else {
            const result = await vscode.window.showOpenDialog({
        ...
        case 'upload': {
          try {
            await this.api.uploadFolder({
              ...
              local_paths: msg.localPaths && msg.localPaths.length > 0 ? msg.localPaths : undefined,
              repo_id: msg.repoId,
...
<div class="field">
  ...
  <div class="hrow">
    ...
    <button class="btn btn-s" id="btnBrowseFiles" title="Select multiple individual files">Files…</button>
  </div>
  <div id="multiFileList" style="display:none;margin-top:6px;padding:6px 10px;background:var(--vscode-input-background);border:1px solid var(--vscode-input-border,#454545);border-radius:3px;font-size:11px;color:var(--vscode-descriptionForeground);max-height:80px;overflow-y:auto"></div>
</div>
...
(function() {
  ...
  // Multi-state for multiple selected files
  let _selectedPaths = [];

  document.getElementById('btnBrowse').addEventListener('click', () => {
    vscode.postMessage({ type: 'browse', browseMode: 'folder' });
  });

  document.getElementById('btnBrowseFiles').addEventListener('click', () => {
    vscode.postMessage({ type: 'browse', browseMode: 'files' });
  });
  ...
  function doUpload() {
    ...
    vscode.postMessage({
      type:          'upload',
      localPath,
      localPaths:    _selectedPaths.length > 0 ? _selectedPaths : undefined,
      repoId,
